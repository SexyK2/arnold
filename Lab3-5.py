...
users = {0: ["nikita", "K2", "admin"]}
lenthusers = len(users)
filedatabase = "NewFile.txt"
...
def createusers(role = False):
    ...
    while reg:
        ...
        if len(login) == 0:
            print("В логине использовать пробел нельзя!")
            login = input("Придумайте логин: ")
        reg = 0
        for i in range(lenthusers):
            if login == users[i][0]:
    ...
    if len(login) != 0:             
        ...
        if len(password) == 0:
            print("В пароле использовать пробел нельзя")
            password = input("Введите пароль без пробела: ")
        if role:
...
def sigin():
    ...
    for i in range(lenthusers):
        if len(password) == 0:
            print('Ваш пароль ну существует')
            os.system('pause')
            mainmenu()
        if (login == users[i][0]) and (password == users[i][1]):
            ...
            if users[i][2] == "admin":
                ...
                print('Увас нет роли, попроситие админа исправить эту ситуацию!')
                os.system('pause')
                mainmenu()
            else:
                usersmod(i)
